...
class MainWindow(QMainWindow):
    ...
    def export_as_pdf(self):
        ...
        if file_path:
            ...
            self.status_bar.show_message(f"Exported as PDF to '{file_path}'.")
    ...
    def open_thesaurus(self):
        ...
        if not selected_text:
            QMessageBox.warning(self, "No Selection", "Please select a word to define.")
            return

        # Fetch definitions using NLTK's WordNet
        synsets = wn.synsets(selected_text)
        if not synsets:
            QMessageBox.information(self, "Definition", f"No definitions found for '{selected_text}'.")
            return
        ...
        for synset in synsets:
            ...
            definition_entry = f"{synset.lexname().split('.')[0].capitalize()}: {definition}"
            if examples:
                definition_entry += f"\nExample: {examples[0]}"
            definitions.append(definition_entry)

        definition_text = '\n\n'.join(definitions)
        QMessageBox.information(self, f"Definition of '{selected_text}'", definition_text)
    ...
    def open_documentation(self):
        """
        Opens the docs HTML page in the default web browser.
        """
        current_dir = os.path.dirname(os.path.abspath(__file__))
        docs_path = os.path.join(current_dir, 'resources', 'docs', 'documentation.html')  # Assuming docs are in a 'docs' folder

        if os.path.exists(docs_path):
            webbrowser.open_new_tab(f'file://{docs_path}')
        else:
            QMessageBox.warning(self, "Documentation Not Found", "The docs file was not found.")
